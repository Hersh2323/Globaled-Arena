function rightClickDotDamage( event )
...
function rightClickDotDamageCritical( event )
	...
	if not target:IsMagicImmune() then
		...
		if damageToPrint > 0 then
			PopupNumbers(target, "poison", Vector(215, 50, 248), 2.0, damageToPrint, nil, POPUP_SYMBOL_POST_EYE)
		else
		end
